...
class DataFramePreprocessor:
            ...
            for line in tqdm(f):
                ...
                for data in line['mention_data']:
                    ...
                    # 非NIL
                        ...
                        if data['kb_id'] not in kbid_to_types.keys():
                            continue
    ...
    def run(self):
        ...
        self.process_type_data(
            input_path=RAW_PATH + 'train.json',
            output_path=TSV_PATH + f'ET_TRAIN_{self.is_nil}.tsv',
        )
        logger.info('Process ET_TRAIN Finish.')
        self.process_type_data(
            input_path=RAW_PATH + 'dev.json',
            output_path=TSV_PATH + f'ET_VALID_{self.is_nil}.tsv',
        )
        logger.info('Process ET_VALID Finish.')
        self.process_type_data(
            input_path=RAW_PATH + 'test.json',
            output_path=TSV_PATH + f'ET_TEST_{self.is_nil}.tsv',
        )
